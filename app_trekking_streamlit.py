...
def naver_search(where: str, query: str, topk: int = 5) -> Dict[str, Any]:
    ...
    try:
        ...
        if where == "blog":
            links = soup.select(".api_txt_lines.total_tit") or soup.select("a.api_txt_lines")
            ...
            descs = soup.select(".news_dsc .dsc_txt_wrap") or soup.select(".api_txt_lines.dsc_txt")
...
def vworld_get_trails(api_key: str, bbox: Tuple[float, float, float, float], size: int = 1000) -> List[Dict[str, Any]]:
    """
...
def aggregate_courses(records: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    rep = df.loc[idx, ["course_id", "coords", "end_lat", "end_lon"]].set_index("course_id")
...
def score_course_row(course_id: str, mntn_nm: str, area_hint: str) -> Dict[str, Any]:
    ...
    score = popularity_score(int(blog.get("total", 0) or 0), int(news.get("total", 0) or 0))
...
def extract_place(el: Dict[str, Any], origin_lat: float, origin_lon: float) -> Optional[Dict[str, Any]]:
    if el.get("type") != "node":
        return None
    tags = el.get("tags") or {}
    name = tags.get("name")
...
def places_near(lat: float, lon: float, radius_m: int) -> List[Dict[str, Any]]:
    ...
    for p in places:
        ...
        p["combined_score"] = round(dist_score * 0.6 + (p["quality_score"] / 5) * 0.4, 3)
...
# Streamlit UI
# ----------------------------
st.set_page_config(page_title="서울 트레킹 추천 (공공데이터+크롤링)", page_icon="🥾", layout="wide")

st.title("🥾 서울 트레킹 코스 추천")
st.caption("공공데이터(등산로) + 네이버 블로그/뉴스 크롤링 근거로 난이도/지역별 Top 코스를 추천합니다.")

with st.sidebar:
    st.header("1) 공공데이터 설정")
    st.caption("VWorld(브이월드) 등산로 API 키가 필요합니다.")
    api_key = st.text_input("VWorld API Key", type="password", placeholder="발급받은 키를 입력")
    ...
    st.header("3) 난이도/정렬")
    diff_filter = st.radio("난이도", ["전체", "쉬움(하)", "보통(중)", "어려움(상)"], horizontal=False)
    topk = st.slider("추천 코스 개수", 3, 10, 3)

    st.header("4) 트레킹 후 추천")
    near_radius_m = st.slider("종료점 주변 추천 반경(m)", 100, 1500, 600, 50)
    sip_choice = st.radio("추천 종류", ["전체", "카페(☕)", "맥주(🍺)"], horizontal=True)
...
with st.status("공공데이터에서 등산로(트레킹 코스 후보) 가져오는 중…", expanded=False) as status:
    try:
...
if courses.empty:
    st.info("선택한 지역에서 코스 후보를 찾지 못했습니다. 반경을 늘리거나 다른 지역을 선택해 보세요.")
    st.stop()
...
courses = courses.sort_values("total_len_m", ascending=False).head(30).reset_index(drop=True)
...
courses["final_score"] = (
    courses["popularity"] * 1.0 + courses["total_len_km"].apply(lambda x: math.log1p(float(x))) * 0.4
).round(3)
...
with col_map:
    ...
    # bbox 표시(대략)
    minx, miny, maxx, maxy = bbox
    folium.Rectangle(bounds=[[miny, minx], [maxy, maxx]], color="#0984e3", weight=2, fill=False).add_to(m)

    line_colors = ["#6c5ce7", "#00b894", "#e17055", "#0984e3", "#d63031", "#e84393", "#2d3436", "#fdcb6e"]

    for i, row in top.iterrows():
        coords = row["coords"]  # [[lon,lat],...]
        latlon = [[c[1], c[0]] for c in coords]
        color = line_colors[i % len(line_colors)]
        folium.PolyLine(latlon, color=color, weight=6, opacity=0.85, tooltip=f"{i+1}위 {row['course_id']}").add_to(m)
...
with col_panel:
    ...
    show_cols = ["course_id", "difficulty", "total_len_km", "est_time_min", "blog_total", "news_total", "final_score"]
...
if not places:
    ...
    keep = ["name", "category", "distance_m", "quality_score", "combined_score", "opening_hours", "website"]
